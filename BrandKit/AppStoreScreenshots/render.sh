...
# 1284x2778 PNGs (Apple's required 6.7"/6.5" iPhone size) in ../../AppStore/screenshots-1284x2778.
# Usage: ./render.sh
set -e
cd "$(dirname "$0")"
CHROME="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
OUT="../../AppStore/screenshots-1284x2778"
mkdir -p "$OUT"
...
for pair in $PAIRS; do
  ...
  "$CHROME" --headless --disable-gpu --force-device-scale-factor=1 --hide-scrollbars \
    --window-size=1284,2778 --screenshot="$(pwd)/$OUT/$out_name.png" "file://$(pwd)/$f.html" \
